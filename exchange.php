...
if (empty($args)) {
    ...
    while (1) {
        if (isset($map['username']) && isset($map['age']) && isset($map['sex']) && isset($map['array'])) {
            break;
        }
        /** 出让cpu，防止长时间占用影响其他任务 */
        usleep(1);
    ...
    var_dump("投递计算结果完成");
    /** 判断数组是否是列表 ，也可能是map */
    var_dump(array_is_list($map['array']));
}
